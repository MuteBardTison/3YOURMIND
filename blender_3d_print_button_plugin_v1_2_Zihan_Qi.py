...
class PrintButtonPanel(bpy.types.Panel):
    ...
    bpy.types.Scene.options_enum = bpy.props.EnumProperty(
        items = [('Together', 'Together', ''),
                 ('Independently', 'Independently', '')],
        name = "Upload")

    #bpy.types.Scene.options_bool = bpy.props.BoolProperty(
        #name = "Upload Independently",
        #description="Upload Independently",
        #default = False
    #)

    def draw(self, context):
        # Checkbox for upload options
        #self.layout.prop(bpy.context.scene, "options_bool")
        # Drop down List for upload options
        self.layout.prop(bpy.context.scene, "options_enum")
        # Button
    ...
    def execute(self, context):
        if(bpy.context.mode == 'OBJECT'):
            ...
            if(len(bpy.context.selected_objects)>0):
                files = []
                open_files = []
                #if(len(bpy.data.objects) == 1 or (len(bpy.data.objects) != 1 and bpy.context.scene.options_bool == False)):
                if(len(bpy.data.objects) == 1 or (len(bpy.data.objects) != 1 and bpy.context.scene.options_enum == 'Together')):
                    ...
                    if bpy.data.is_saved == False:
                        ...
                        filename=somename[:-6]
                    files.append(("file", (filename + ".stl", file, 'application/sla')))
                    ...
                    for stl_file in os.listdir(path):
                        if stl_file.endswith(".stl") and (not stl_file.lower().startswith(("camera", "lamp"))):
                            file = open(path + stl_file)
                            open_files.append(file)
                            files.append(("file", (filename + str(index) + ".stl", file, 'application/sla')))
                ...
                for f in files:
                    try:
                        r = requests.post(url, files={f[0]:f[1]}, data=fields, headers=custom_header, verify=False)
